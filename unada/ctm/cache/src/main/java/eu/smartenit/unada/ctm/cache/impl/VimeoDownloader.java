...
/**
 * The VimeoDownloader runnable class. 
 * It includes methods that retrieve content information from Vimeo, 
...
public class VimeoDownloader implements VideoDownloader, Runnable {
	...
	private static final Logger logger = LoggerFactory.getLogger(VimeoDownloader.class);
	...
	/**
	 * The method that downloads the video from Vimeo. 
	...
	public void downloadVideo() throws Exception {
		...
		logger.info("Attempting to download content " + contentID + " from Vimeo.");
        ...
        if (content != null) {
            logger.info("Content " + contentID + " is either downloading or already cached.");
            return;
        }
		else {
			...
			try {
                if (target == null) {
                    throw new Exception("Streaming URL not according to pattern.");
                }
            ...
            UnadaThreadService.getThreadService().execute(new Runnable() {
                @Override
                public void run() {
                    OverlayFactory.getOverlayManager().advertiseContent(contentID);
                }
            });
		}
	 ...
	 * 
	 * @param v The video information, as extracted by Vimeo.
	 * 
