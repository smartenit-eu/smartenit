...
public class ContentManagerImpl implements ContentManager {
	...
	public void deleteContent(Content content) {
		if (content != null) {
			...
			//String contentRootDirectory = FileUtils.getRootDir(content.getPath());
	...
	public void deleteContents(List<Content> contents) {
		if (contents != null && contents.size() != 0) {
			...
			for (Content c : contents) {
				//String contentRootDirectory = FileUtils.getRootDir(c.getPath());
    ...
    public void deleteContents(List<Content> contents, long threshold) {
        if (contents != null && contents.size() != 0) {
            logger.info("Deleting least accessed content list for threshold of " + threshold + "ms.");
            for (Content c : contents) {
                boolean deleted = DAOFactory.getContentDAO().deleteNotAccessedContent(c,
		...
		if (content != null) {
			if (content.getPath() != null && !content.getPath().isEmpty()) {
				logger.info("Content " + content.getContentID()
                        + " includes path, hence it's from overlay prediction. Hops = "
                        + content.getHopCount());

                if (content.getHopCount() <= MAX_HOPS) {
                    logger.info("Content is closer from the overlay, hence will be fetched from there.");
                    OverlayDownloader overlayDownloader = new OverlayDownloader(content);
				...
				logger.info("Content " + content.getContentID()
                        + " does not include path, hence it's from social prediction.");
				VimeoDownloader vimeoDownloader = new VimeoDownloader(
						"http://www.vimeo.com/m/" + content.getContentID(),
						true);
				setVimeoDownloader(vimeoDownloader);
