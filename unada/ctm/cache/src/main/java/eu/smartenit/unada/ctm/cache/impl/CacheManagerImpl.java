...
public class CacheManagerImpl implements CacheManager {
	...
	public void updateCache(List<Content> socialContentList,
			...
			if (index < socialListSize) {
                ...
                if (!toBePrefetched.contains((Long) c.getContentID())) {
                    ...
                    if (overlayIndex != -1) {
                        ...
                        // if overlay score is higher than threshold, then prefer
                        // overlay
                        if (overlayContentList.get(overlayIndex).getCacheScore() >= cache
                                .getOverlayThreshold()) {
                        ...
                        if (c.getCacheScore() < cache.getSocialThreshold()
                            ...
                            logger.debug("SL: Adding content " + c.getContentID()
                                    + " to ignored list.");
                            ignoredContentList.add(c);
                        } else {
                            logger.debug("SL: Attempting to prefetch content "
		...
		logger.info("Updated cache size " + (float)currentCacheSize / 1000000 + "MB.");

        // Finally delete the ignored ones
        // Check if cache is very close to threshold: delete all ignored, otherwise delete least accessed.
        if (currentCacheSize > (float) cache.getSize() - 1.5 * (float) cache.getSizeThreshold()) {
            ...
            getContentManager().deleteContents(ignoredContentList, cache.getTimeToLive());
        }
	}
	...
	public void updateSocialContentListWithSizes(List<Content> contentList) {
					...
					new Callable<VideoInfo>() {
						...
						public VideoInfo call() throws Exception {
							return new VimeoInfoRetriever()
									.retrieveVideoInfo("http://www.vimeo.com/" + contentID);
						}
					});
			futureList.add(f);
		}
		...
		for (int i = 0; i < futureList.size(); i++) {
			...
			try {
				...
				logger.error("Interruption while retrieving info: " + e.getMessage());
			...
			if (info != null) {
                ...
                logger.debug("Content " + contentList.get(i).getContentID() + ", size = " + info.getInfo().getLength());
			}
            else {
