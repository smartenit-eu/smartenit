...
public class UnadaSessionBean {
	...
	public void init() {
        ...
        try {
            noFacebook = (Boolean) FacesContext.getCurrentInstance().getExternalContext()
                    .getApplicationMap().get("noFacebook");
            if (noFacebook) {
                name = "admin";
                return;
            }
	...
	private String retrieveToken(String code) throws Exception {
		...
		try {
			...
			String[][] parameters = {
					...
					{ "client_secret",
						UnadaConstants.APP_SECRET },
					{
							"redirect_uri", UnadaConstants.REDIRECT_URI
									+ "/unada/index.xhtml" }, { "code", code } };
