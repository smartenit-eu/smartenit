...
public class LogToFileSecondVersionTest {
	...
	public void init() {
		...
		log.append(extendedSNMPTrafficCollector2.logToFileSecondVersion(AS_NUMBER, prepareValues(100, 1000)));
		log.append(extendedSNMPTrafficCollector2.logToFileSecondVersion(AS_NUMBER, prepareValues(200, 2000)));
		log.append(extendedSNMPTrafficCollector2.logToFileSecondVersion(AS_NUMBER, prepareValues(300, 3000)));
		log.append(extendedSNMPTrafficCollector2.logToFileSecondVersion(AS_NUMBER, prepareValues(400, 4000)));
	}
	...
	private String removeTimestamp(String log) {
		...
		for (int line = 0; line < lines.length; line++) {
			if(lines[line].startsWith("R") || lines[line].startsWith("A")) {
				response.append(lines[line].substring(15)).append("\n");
			}
		}
	...
	protected ExtendedCounterValues prepareValues(int linkPackets, int tunnelPackets) {
		...
		values.storeCounterValue(new SimpleLinkID("211", "isp1"), linkPackets + 5);
		values.storeReceivedPackets(new SimpleLinkID("211", "isp1"), new ReceivedPackets(linkPackets + 5 + 50L, linkPackets + 5 + 100L, linkPackets + 5 + 150L));
		...
		final EndAddressPairTunnelID tunnelID8 = new EndAddressPairTunnelID("tunnel2111", 
