...
/**
 * Used to store {@link XVector} and {@link RVector} objects representing link traffic 
...
public class VectorsContainer {
	...
	/**
	 * Stores new reference vector ({@link RVector}) in the container. If
	 ...
	 * @return updated pair object
	 */
	public XRVectorPair storeUpdatedRVector(RVector rVector) {
	...
	private void insertNewVectorPairWithRVector(int asNumber, RVector rVector) {
	...
	private void accumulateAndUpdateExistingVectorPairWithXVector(XRVectorPair pair, XVector xVector) {
		...
		for (VectorValue currentValue : pair.getXVector().getVectorValues()){
	...
	private void updateExistingVectorPairRVector(XRVectorPair pair,	RVector rVector) {
