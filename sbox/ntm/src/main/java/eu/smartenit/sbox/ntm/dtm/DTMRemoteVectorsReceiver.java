...
public class DTMRemoteVectorsReceiver {
	...
	private void configureAllControllers() {
		...
		List<DC2DCCommunication> inCommunications = new ArrayList<DC2DCCommunication>(); 
		List<DC2DCCommunication> outCommunications = new ArrayList<DC2DCCommunication>();
		...
		for(DC2DCCommunication communication : DAOFactory.getDCDC2DCCommunicationDAOInstance().findAllDC2DCCommunicationsCloudsTunnels()) {
			if(communication.getTrafficDirection().equals(Direction.incomingTraffic))
				inCommunications.add(communication);
			else
				outCommunications.add(communication);
		}
		ConfigData configData = new ConfigData(inCommunications, outCommunications);
		
		SDNConfigPusher configPusher = new SDNConfigPusher(controllers.getAllControllers());
		configPusher.initializeSDNController(configData);
		logger.info("... done");
	}
