...
public class OneRecDCWith95thPercentileUpdateControlRVectorAchievedTest {
	
	private static final Logger logger = LoggerFactory.getLogger(OneRecDCWith95thPercentileUpdateControlRVectorAchievedTest.class);	
	 ...
	 * known values. C vector updates are sent only if specific conditions are
	 * met. Before the end of accounting period, ECA sends to NTM a list of
	 * links for which the calculated R vector value has already been achieved.
	 * This triggers different C vector calculation algorithm.
	 */
	@Test @Ignore
	public void shouldTestCompleteWorkflowWithGivenValuesAnd95thPercentileChargingRule() throws Exception {
		...
		verify(mockedSDNClient, times(262)).distribute(any(SDNController.class), cVectorArgument1.capture());
	...
	private void verifyAllCVectorUpdates(ArgumentCaptor<CVector> cVectorArgument1) {
		...
		int noOfVectors = cVectorArgument1.getAllValues().size();
		
		assertEquals(3120000, cVectorArgument1.getAllValues().get(noOfVectors-1).getVectorValueForTunnelEndPrefix(tunnelEndPrefix1));
		assertEquals(-3120000, cVectorArgument1.getAllValues().get(noOfVectors-1).getVectorValueForTunnelEndPrefix(tunnelEndPrefix2));
	...
	private void verifyAllCRVectorUpdates(ArgumentCaptor<CVector> cVectorArgument2, ArgumentCaptor<RVector> rVectorArgument) {
		...
		assertEquals(18480, rVectorArgument.getAllValues().get(0).getVectorValueForTunnelEndPrefix(tunnelEndPrefix1));
		assertEquals(12720, rVectorArgument.getAllValues().get(0).getVectorValueForTunnelEndPrefix(tunnelEndPrefix2));
		...
		assertEquals(18480, rVectorArgument.getAllValues().get(1).getVectorValueForTunnelEndPrefix(tunnelEndPrefix1));
		assertEquals(12720, rVectorArgument.getAllValues().get(1).getVectorValueForTunnelEndPrefix(tunnelEndPrefix2));
	}
	...
	private void prepareMockForDAOAtNTM(boolean receiver) {
		...
		when(ntmMockedTimeDAO.findLast()).thenReturn(DBStructuresBuilderForMockedDAO.tspForLong95thPercentileRule);
	...
	private void prepareMockForDAOAtQOA() {
		...
		when(qoaMockedTimeDAO.findLast()).thenReturn(DBStructuresBuilderForMockedDAO.tspForLong95thPercentileRule);
	...
	private void prepareMockForDAOAtECA() {
		...
		when(ecaMockedTimeDAO.findLast()).thenReturn(DBStructuresBuilderForMockedDAO.tspForLong95thPercentileRule);
	...
	private void prepareMockForSNMPWrapper(SNMPWrapper snmpWrapper) {
		...
		when(snmpWrapper.snmpGet(Mockito.argThat(new IsOidEqual("1.3.6.1.2.1.31.1.1.1.6.0")), Mockito.argThat(new IsAddressEqual("1.2.1.1"))))
			.thenAnswer(new Answer<String>() {
				...
				public String answer(InvocationOnMock invocation) throws Throwable {
					traffic += (5000 + 100*(count++ %10));
					return "1.3.6.1.2.1.31.1.1.1.6.0 = " + traffic + "]";
				}
			});
		...
		when(snmpWrapper.snmpGet(Mockito.argThat(new IsOidEqual("1.3.6.1.2.1.31.1.1.1.6.1")), Mockito.argThat(new IsAddressEqual("1.2.1.2"))))
			.thenAnswer(new Answer<String>() {
				...
				@Override
				public String answer(InvocationOnMock invocation) throws Throwable {
					traffic += (4000 + 100*(count++ %8));
					return "1.3.6.1.2.1.31.1.1.1.6.1 = " + traffic + "]";
				}
			});
	...
	private LocalRVector prepareRVectorWithInitialValues() {
    	...
    	rVector.addVectorValueForLink(new SimpleLinkID("111", "isp1"), 19000L);
    	rVector.addVectorValueForLink(new SimpleLinkID("121", "isp1"), 10000L);
    	return rVector;
	}
